...
AFDD-class arc work is a **research lens**, not a certified SKU. Capture remains ISR-tied (not an arc band). Separate HF path (`ADC_ETC`+DMA or external 16-bit **M3**), AFE, dither/detect mutex, FlexPWM carrier blanking, and dual-MCU are required before any safety argument; Teensy HF/MACAPD scores must not drive OUTEN. Preferred M3 parts: **TI ADS8860** (single HF I), **ADI AD7380** (dual simultaneous I+V). Host math: `src/afdd_macapd.h` + `test/test_afdd_macapd` (unused by OUTEN). Notes: `docs/FEATURE_AFDD_RESEARCH_2026-08-30.md`, `docs/FEATURE_AFDD_MACAPD_ALGORITHM.md`, `docs/PRODUCT_READINESS.md` §1. Do not add UL 1699B or “arc protection” product claims. Do not open `plan/refactor-adversarial-fixes-7.md` for this.
