...
AFDD-class arc work is a **research lens**, not a certified SKU. Capture remains ISR-tied (not an arc band). Separate HF path (`ADC_ETC`+DMA or external 16-bit), AFE, dither/detect mutex, FlexPWM carrier blanking, and dual-MCU are required before any safety argument; Teensy HF scores must not drive OUTEN. Durable notes: `docs/FEATURE_AFDD_RESEARCH_2026-08-30.md` (MACAPD), `docs/PRODUCT_READINESS.md` §1. Do not add UL 1699B or “arc protection” product claims. Do not open `plan/refactor-adversarial-fixes-7.md` for this.
