...
README operator UI stills: `docs/images/readme-ui-*.png`, regenerated by `scripts/readme_ui_fixtures/serve_and_capture.py` (local fixture `/api`, not Teensy). Captions must say UI orientation / fixture / inhibited — not bench proof / not ISR/OUTEN proof. Do not put gallery media in `web/`; prefer PNG stills over animated live-meter GIFs. Plan `plan/process-readme-screenshots-1.md` is Completed; still no `plan/refactor-adversarial-fixes-7.md`.
