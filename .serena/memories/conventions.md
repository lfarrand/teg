...
Host six-suite gate: serde/ota/spectrum/thermal_math/waveform/features plus `test_spectrum_wire_quantize_saturates`, not ISR/OUTEN. Also native: `test_mqtt_discovery` (8/8) — report separately. Do not publish a 91/99/six-suite numeric total; dated REVIEW_FIXES 91/91 lines stay as snapshots. #69 landed on main as the docs-only operator-copy alignment; land further operator-doc / comment / memory refreshes on a feature-branch PR, not a commit on `main`.
