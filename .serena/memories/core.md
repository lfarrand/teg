...
This is a bench instrument: HTTP/MQTT/Influx have no TLS; the PIN is cleartext. PLL is a bench reference lock, not grid-tie. Operator README/BENCH_CHECKS/SECURITY/CI_SECURITY/PRODUCT_READINESS/`web/index.html`/teg-pwm-memory name landed #67/#68 contracts; they are not bench proof. Production `TEG_ENABLE_UNSAFE_LAB_OTA` is undefined (`ota.h` stubs, empty `ota.cpp`/`flash_ota.cpp`, no `/api/ota` routes). Isolation assumptions: `docs/SECURITY.md`. Product-vs-instrument gap: `docs/PRODUCT_READINESS.md`.
