...
auto filename = "/settings.cfg"; // <- SD library uses 8.3 filenames
MainConfig config;
...
constexpr uint16_t MAX_COUNTER_VALUE = 0xFFFF; // 16-bit counter maximum (65535)
...
void logPwmModuleStats(const char* moduleId, auto& pwmModule, bool hasChanB = true);
...
static const char PROGMEM PwmTimerSettingsPageTemplate[] = R"delimiter(<!DOCTYPE html>
<html lang="en">
<head>
...
static const char PROGMEM PwmSettingsPageTemplate[] = R"delimiter(<!DOCTYPE html>
<html lang="en">
<head>
...
void configurePwmSyncTimer() {
  ...
  if (config.Pwm.SyncPwm) {
    ...
    pwmSyncTimer.begin([] {
    }, 1us, true); // 1MHz timer used to synchronise PWM modules
  }
}
...
void index(Request &req, Response &res) {
  ...
  char timeText[20];
  snprintf(timeText, sizeof(timeText), "%d-%02d-%02d %02d:%02d:%02d", year(), month(), day(), hour(), minute(),
           second());
...
void loop() {
  ...
  */
  ...
  if (millis() - lastRamCheck >= 1000) {  // Check every 1 second
    int freeRam = getFreeMemory();
    char buf[50];
    snprintf(buf, sizeof(buf), "Free RAM: %d bytes", freeRam);
    writeLog(buf);  // Displays on OLED and Serial
    // Optional: Send to InfluxDB
    String influxData = String("memory free_ram=") + freeRam;
    writeInfluxDb(influxData);
    lastRamCheck = millis();
  }
...
void pollConfigSettings() {
  ...
  snprintf(temp, sizeof(temp),
           "configSettings,setting_id=spwmModulationFrequency,setting_category=pwm value=%s %llu\n", buffer,
           timestamp);
  postRequest += temp;
  ...
  ultoa(config.AsymmetricInduction.IsEnabled, buffer, DEC);
  snprintf(temp, sizeof(temp),
           "configSettings,setting_id==isEnabled,setting_category=asymmetricinduction value=%s %llu\n", buffer,
           timestamp);
  postRequest += temp;

  ultoa(config.AsymmetricInduction.PreShiftNanos, buffer, DEC);
  snprintf(temp, sizeof(temp),
           "configSettings,setting_id=preShiftNanos,setting_category=asymmetricinduction value=%s %llu\n", buffer,
           timestamp);
  postRequest += temp;

  ultoa(config.AsymmetricInduction.PostShiftNanos, buffer, DEC);
  snprintf(temp, sizeof(temp),
           "configSettings,setting_id=postShiftNanos,setting_category=asymmetricinduction value=%s %llu\n", buffer,
           timestamp);
  postRequest += temp;
...
void configureModule1() {
  ...
  */
  ...
  const uint16_t deadTimeCycles = (static_cast<uint64_t>(Tm1.srcClockHz()) * config.Pwm.Tm1.Sm13.DeadTime) /
                                  1000000000;
  Sm13.setupDeadtime(deadTimeCycles);

  char strBuf[LOG_BUF_SIZE] = {0};
  ret = snprintf(strBuf, sizeof(strBuf), "Set TM1 deadtime to %hu cycles", deadTimeCycles);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured SM13 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
...
void configureModule2() {
  ...
  */
  ...
  if (config.Pwm.Tm2.UseSpwm) {
    ...
    ret = snprintf(strBuf, sizeof(strBuf),
                   "SPWM carrier frequency: %luHz, modulation frequency: %luHz, ratio: %lf, update speed: %lf",
                   config.Pwm.Tm2.SpwmCarrierFrequency, config.Pwm.Tm2.SpwmModulationFrequency, spwmRatio,
                   spwmUpdateSpeed);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting SM20 ChanA duty cycle to %u",
                   config.Pwm.Tm2.Sm20.ChannelA.DutyCycle);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting SM20 ChanB duty cycle to %u",
                   config.Pwm.Tm2.Sm20.ChannelB.DutyCycle);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    if (config.Pwm.Tm2.Sm21.ChannelA.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM21 ChanA phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm2.Sm21.ChannelA.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "Setting SM21 ChanA duty cycle to %u",
                     config.Pwm.Tm2.Sm21.ChannelA.DutyCycle);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
    ...
    if (config.Pwm.Tm2.Sm22.ChannelA.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM22 ChanA phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm2.Sm22.ChannelA.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "Setting SM22 ChanA duty cycle to %u",
                     config.Pwm.Tm2.Sm22.ChannelA.DutyCycle);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
    ...
    if (config.Pwm.Tm2.Sm22.ChannelB.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM22 ChanB phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm2.Sm22.ChannelB.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "Setting SM22 ChanB duty cycle to %u",
                     config.Pwm.Tm2.Sm22.ChannelB.DutyCycle);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
    ...
    if (config.Pwm.Tm2.Sm23.ChannelA.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM23 ChanA phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm2.Sm23.ChannelA.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "Setting SM23 ChanA duty cycle to %u",
                     config.Pwm.Tm2.Sm23.ChannelA.DutyCycle);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
    ...
    if (config.Pwm.Tm2.Sm23.ChannelB.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM23 ChanB phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm2.Sm23.ChannelB.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "Setting SM23 ChanB duty cycle to %u",
                     config.Pwm.Tm2.Sm21.ChannelB.DutyCycle);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set Sm20 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set Sm21 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "#4 PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale,
                 pwmMode,
                 deadtimeSettingChanA, dutyCycleSettingChanA);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set Sm22 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set Sm23 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "#5 PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale,
                 pwmMode,
                 deadtimeSettingChanA, dutyCycleSettingChanA);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  if(Sm21.setupPwmPhaseShift(ChanA, config.Pwm.Tm2.Sm21.ChannelA.PhaseShift, true)) {
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM21 ChanA succeeded", config.Pwm.Tm2.Sm21.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
  writeLog("Log buffer overflow!");
} else {
  writeLog(strBuf);
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM21 ChanA failed", config.Pwm.Tm2.Sm21.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
  writeLog("Log buffer overflow!");
} else {
  writeLog(strBuf);
  ...
  */
  ...
  if (Sm22.setupPwmPhaseShift(ChanA, config.Pwm.Tm2.Sm22.ChannelA.PhaseShift, true)) {
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM22 ChanA succeeded",
                   config.Pwm.Tm2.Sm22.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM22 ChanA failed",
                   config.Pwm.Tm2.Sm22.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  if (Sm22.setupPwmPhaseShift(ChanB, config.Pwm.Tm2.Sm22.ChannelB.PhaseShift, true)) {
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM22 ChanB succeeded",
                   config.Pwm.Tm2.Sm22.ChannelB.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM22 ChanB failed",
                   config.Pwm.Tm2.Sm22.ChannelB.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  if (Sm23.setupPwmPhaseShift(ChanA, config.Pwm.Tm2.Sm23.ChannelA.PhaseShift, true)) {
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM23 ChanA succeeded",
                   config.Pwm.Tm2.Sm23.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM23 ChanA failed",
                   config.Pwm.Tm2.Sm23.ChannelA.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  if (Sm23.setupPwmPhaseShift(ChanB, config.Pwm.Tm2.Sm23.ChannelB.PhaseShift, true)) {
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM23 ChanB succeeded",
                   config.Pwm.Tm2.Sm23.ChannelB.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting phase shift to %u for SM23 ChanB failed",
                   config.Pwm.Tm2.Sm23.ChannelB.PhaseShift);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 2.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  dutyCycleSettingChanA = Sm21.dutyCycleSetting(ChanA);
  prescale = prescaleStr[Sm21.prescaler()];
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency,
                 prescale, pwmMode,
                 deadtimeSettingChanA, dutyCycleSettingChanA);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 2.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 2.3 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
...
void configureModule3() {
  ...
  */
  ...
  const uint16_t deadTimeCycles = (static_cast<uint64_t>(Tm3.srcClockHz()) * config.Pwm.Tm3.Sm31.DeadTime) /
                                  1000000000;
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set TM3 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured SM31 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
...
void configureModule4() {
  ...
  */
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set TM4.0 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Set TM4.1 deadtime to %hu cycles", deadTimeCycles);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
  ...
  if (!config.AsymmetricInduction.IsEnabled) {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Set TM4.2 deadtime to %hu cycles", deadTimeCycles);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
      writeLog(strBuf);
  ...
  if (!config.AsymmetricInduction.IsEnabled) {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting SM42 ChanA duty cycle to %u",
                   config.Pwm.Tm4.Sm42.ChannelA.DutyCycle);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting SM42 ChanB duty cycle to %u",
                   config.Pwm.Tm4.Sm42.ChannelB.DutyCycle);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  if (config.Pwm.Tm4.Sm41.ChannelA.PhaseShift != 0) {
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "SM41 ChanA phase shift was %u, setting duty cycle to %u",
                   config.Pwm.Tm4.Sm41.ChannelA.PhaseShift, FIFTY_PERCENT_DUTY);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
      writeLog(strBuf);
    ...
    ret = snprintf(strBuf, sizeof(strBuf), "Setting SM41 ChanA duty cycle to %u",
                   config.Pwm.Tm4.Sm41.ChannelA.DutyCycle);
    if (ret < 0 || ret >= sizeof(strBuf)) {
      writeLog("Log buffer overflow!");
    } else {
  ...
  if (!config.AsymmetricInduction.IsEnabled) {
    if (config.Pwm.Tm4.Sm42.ChannelA.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM42 ChanA phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm4.Sm42.ChannelA.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
    ...
    if (config.Pwm.Tm4.Sm42.ChannelB.PhaseShift != 0) {
      ...
      ret = snprintf(strBuf, sizeof(strBuf), "SM42 ChanB phase shift was %u, setting duty cycle to %u",
                     config.Pwm.Tm4.Sm42.ChannelB.PhaseShift, FIFTY_PERCENT_DUTY);
      if (ret < 0 || ret >= sizeof(strBuf)) {
        writeLog("Log buffer overflow!");
      } else {
        writeLog(strBuf);
...
*/
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 4.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency,
                 prescale, pwmMode,
                 deadtimeSettingChanA, dutyCycleSettingChanA);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 4.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency,
                 prescale, pwmMode,
                 deadtimeSettingChanA, dutyCycleSettingChanA);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "Configured PWM 4.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                 pwmFrequency, prescale,
                 pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
...
uint8_t calculateBestPrescaler(uint32_t pwmFrequency) {
  ...
  for (uint8_t i = 0; i < numPrescalers; i++) {
    ...
    // Check if the period fits within the 16-bit counter range
    if (periodTicks <= MAX_COUNTER_VALUE) {
      return i; // Return the prescaler index for use in FLEXPWM_CTRL_PRSC
    }
  }

  // If no valid prescaler is found, return the largest prescaler index
...
FASTRUN void IsrOverflowSm20() {
  ...
  if (++vSample >= UINT32_MAX) vSample = 0;  // Prevent overflow wrap-around issues
...
bool xbarConnect(uint8_t input, uint8_t output) {
  ...
  */
  ...
  ret = snprintf(strBuf, sizeof(strBuf), "  Writing value 0x%04X to register XBARA1_SEL%hu (address 0x%" PRIXPTR ")",
                 val, (output / 2),
                 reinterpret_cast<uintptr_t>(&XBARA1_SEL0 + (output / 2)));
  if (ret < 0 || ret >= sizeof(strBuf)) {
    writeLog("Log buffer overflow!");
  } else {
    writeLog(strBuf);
...
void logPwmModuleStats(const char* moduleId, auto& pwmModule, bool hasChanB) {
  char strBuf[LOG_BUF_SIZE] = {0};

  uint32_t frequency = pwmModule.pwmFrequency();
  uint32_t mode = pwmModule.pwmMode();
  uint16_t deadtimeA = pwmModule.deadtimeSetting(ChanA);
  uint16_t dutyCycleA = pwmModule.dutyCycleSetting(ChanA);
  const char* prescale = prescaleStr[pwmModule.prescaler()];

  int ret;
  if (hasChanB) {
    uint16_t deadtimeB = pwmModule.deadtimeSetting(ChanB);
    uint16_t dutyCycleB = pwmModule.dutyCycleSetting(ChanB);
    ret = snprintf(strBuf, sizeof(strBuf),
                  "%s %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%",
                  moduleId, frequency, prescale, mode,
                  deadtimeA, dutyCycleA, deadtimeB, dutyCycleB);
  } else {
    ret = snprintf(strBuf, sizeof(strBuf),
                  "%s %luHz %s md:%lu dtA:%hu dcA:%hhu%%",
                  moduleId, frequency, prescale, mode, deadtimeA, dutyCycleA);
  }
...
void printStats() {
  ...
  for (const auto& config : modules) {
    logPwmModuleStats(config.id, config.module, config.hasChanB);
  }
}
...
void loadConfiguration(const char *filename, MainConfig &config) {
  ...
  JsonObject Config_AsymmetricInduction = doc[F("Config")][F("AsymmetricInduction")];
  config.AsymmetricInduction.IsEnabled = Config_AsymmetricInduction[F("IsEnabled")] | false;
  config.AsymmetricInduction.PreShiftNanos = Config_AsymmetricInduction[F("PreShiftNanos")] | 500;
  config.AsymmetricInduction.PostShiftNanos = Config_AsymmetricInduction[F("PostShiftNanos")] | 500;
  ...
  config.Pwm.Tm1.Sm13.ChannelA.OnPeriodMicroseconds =
      Config_Pwm_Tm1_Sm13[F("ChannelA")][F("OnPeriodMicroseconds")] | 1000;
  ...
  config.Pwm.Tm1.Sm13.ChannelB.OnPeriodMicroseconds =
      Config_Pwm_Tm1_Sm13[F("ChannelB")][F("OnPeriodMicroseconds")] | 1000;
...
void saveConfiguration(const char *filename, const MainConfig &config) {
  ...
  auto Config_AsymmetricInduction = doc[F("Config")][F("AsymmetricInduction")].to<JsonObject>();
  Config_AsymmetricInduction[F("IsEnabled")] = config.AsymmetricInduction.IsEnabled;
  Config_AsymmetricInduction[F("PreShiftNanos")] = config.AsymmetricInduction.PreShiftNanos;
  Config_AsymmetricInduction[F("PostShiftNanos")] = config.AsymmetricInduction.PostShiftNanos;

  auto Config_Pwm = doc[F("Config")][F("Pwm")].to<JsonObject>();
  Config_Pwm[F("PrintRegs")] = config.Pwm.PrintRegs;

  auto Config_Pwm_Tm1_Sm13 = Config_Pwm[F("Tm1")][F("Sm13")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm1_Sm13_ChannelA = Config_Pwm_Tm1_Sm13[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm1_Sm13_ChannelB = Config_Pwm_Tm1_Sm13[F("ChannelB")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2 = Config_Pwm[F("Tm2")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm20 = Config_Pwm_Tm2[F("Sm20")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm20_ChannelA = Config_Pwm_Tm2_Sm20[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm20_ChannelB = Config_Pwm_Tm2_Sm20[F("ChannelB")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm21 = Config_Pwm_Tm2[F("Sm21")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm21_ChannelA = Config_Pwm_Tm2_Sm21[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm22 = Config_Pwm_Tm2[F("Sm22")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm22_ChannelA = Config_Pwm_Tm2_Sm22[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm22_ChannelB = Config_Pwm_Tm2_Sm22[F("ChannelB")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm23 = Config_Pwm_Tm2[F("Sm23")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm23_ChannelA = Config_Pwm_Tm2_Sm23[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm2_Sm23_ChannelB = Config_Pwm_Tm2_Sm23[F("ChannelB")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm3_Sm31 = Config_Pwm[F("Tm3")][F("Sm31")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm3_Sm31_ChannelA = Config_Pwm_Tm3_Sm31[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm3_Sm31_ChannelB = Config_Pwm_Tm3_Sm31[F("ChannelB")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4 = Config_Pwm[F("Tm4")].to<JsonObject>();

  auto Config_Pwm_Tm4_Sm40 = Config_Pwm_Tm4[F("Sm40")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm40_ChannelA = Config_Pwm_Tm4_Sm40[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm41 = Config_Pwm_Tm4[F("Sm41")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm41_ChannelA = Config_Pwm_Tm4_Sm41[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm42 = Config_Pwm_Tm4[F("Sm42")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm42_ChannelA = Config_Pwm_Tm4_Sm42[F("ChannelA")].to<JsonObject>();
  ...
  auto Config_Pwm_Tm4_Sm42_ChannelB = Config_Pwm_Tm4_Sm42[F("ChannelB")].to<JsonObject>();
