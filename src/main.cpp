...
static constexpr char PwmTimerSettingsPageTemplate[] = R"delimiter(<!DOCTYPE html>
<html lang="en">
<head>
...
static constexpr char PwmSettingsPageTemplate[] = R"delimiter(<!DOCTYPE html>
<html lang="en">
<head>
...
void pollConfigSettings() {
    ...
    snprintf(temp, sizeof(temp), "configSettings,setting_id==isEnabled,setting_category=asymmetricinduction value=%s %llu\n", buffer, timestamp);
    postRequest += temp;

    ultoa(config.AsymmetricInduction.PreShiftNanos, buffer, DEC);
    snprintf(temp, sizeof(temp), "configSettings,setting_id=preShiftNanos,setting_category=asymmetricinduction value=%s %llu\n", buffer, timestamp);
    postRequest += temp;

    ultoa(config.AsymmetricInduction.PostShiftNanos, buffer, DEC);
    snprintf(temp, sizeof(temp), "configSettings,setting_id=postShiftNanos,setting_category=asymmetricinduction value=%s %llu\n", buffer, timestamp);
    postRequest += temp;
...
void configureModule1() {
    ...
    */
    ...
    sprintf(strBuf, "Configured SM13 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
...
void configureModule2() {
    ...
    */
    ...
    if (config.Pwm.Tm2.UseSpwm) {
        ...
        sprintf(strBuf, "SPWM carrier frequency: %luHz, modulation frequency: %luHz, ratio: %lf, update speed: %lf",
        ...
        if (config.Pwm.Tm2.Sm21.ChannelA.PhaseShift != 0) {
            ...
            sprintf(strBuf, "SM21 ChanA phase shift was %u, setting duty cycle to %u",
        ...
        if (config.Pwm.Tm2.Sm22.ChannelA.PhaseShift != 0) {
            ...
            sprintf(strBuf, "SM22 ChanA phase shift was %u, setting duty cycle to %u",
        ...
        if (config.Pwm.Tm2.Sm22.ChannelB.PhaseShift != 0) {
            ...
            sprintf(strBuf, "SM22 ChanB phase shift was %u, setting duty cycle to %u",
        ...
        if (config.Pwm.Tm2.Sm23.ChannelA.PhaseShift != 0) {
            ...
            sprintf(strBuf, "SM23 ChanA phase shift was %u, setting duty cycle to %u",
        ...
        if (config.Pwm.Tm2.Sm23.ChannelB.PhaseShift != 0) {
            ...
            sprintf(strBuf, "SM23 ChanB phase shift was %u, setting duty cycle to %u",
    ...
    sprintf(strBuf, "#4 PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode,
    ...
    sprintf(strBuf, "#5 PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    */
    ...
    sprintf(strBuf, "Configured PWM 2.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
            pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    dutyCycleSettingChanA = Sm21.dutyCycleSetting(ChanA);
    prescale = prescaleStr[Sm21.prescaler()];
    sprintf(strBuf, "Configured PWM 2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    sprintf(strBuf, "Configured PWM 2.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
            pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    sprintf(strBuf, "Configured PWM 2.3 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
            pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
...
void configureModule3() {
    ...
    */
    ...
    sprintf(strBuf, "Configured SM31 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
            pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
...
void configureModule4() {
    ...
    */
    ...
    if (config.Pwm.Tm4.Sm41.ChannelA.PhaseShift != 0) {
        ...
        sprintf(strBuf, "SM41 ChanA phase shift was %u, setting duty cycle to %u",
    ...
    sprintf(strBuf, "Configured PWM 4.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    sprintf(strBuf, "Configured PWM 4.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
    ...
    sprintf(strBuf, "Configured PWM 4.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale,
            pwmMode, deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf));
...
bool xbarConnect(uint8_t input, uint8_t output) {
    ...
    sprintf(strBuf, "  Value of XBARA1_SEL%hu (address 0x%" PRIXPTR ") register before writing is 0x%04X", (output / 2),
    ...
    */
    ...
    sprintf(strBuf, "  Writing value 0x%04X to register XBARA1_SEL%hu (address 0x%" PRIXPTR ")", val, (output / 2),
    ...
    *xbar_select_reg = val;
    ...
    sprintf(strBuf, "  Value of XBARA1_SEL%hu (address 0x%" PRIXPTR ") register after writing is 0x%04X", (output / 2),
            reinterpret_cast<uintptr_t>(&XBARA1_SEL0 + (output / 2)), *xbar_select_reg);
    writeLog(strBuf);
    /*
...
void printStats() {
    ...
    const char *prescale = prescaleStr[Sm13.prescaler()];
    sprintf(strBuf, "1.3 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    ...
    prescale = prescaleStr[Sm20.prescaler()];
    sprintf(strBuf, "2.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    dutyCycleSettingChanA = Sm21.dutyCycleSetting(ChanA);
    prescale = prescaleStr[Sm21.prescaler()];
    sprintf(strBuf, "2.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode, deadtimeSettingChanA,
            dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    dutyCycleSettingChanB = Sm22.dutyCycleSetting(ChanB);
    prescale = prescaleStr[Sm22.prescaler()];
    sprintf(strBuf, "2.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    dutyCycleSettingChanB = Sm23.dutyCycleSetting(ChanB);
    prescale = prescaleStr[Sm23.prescaler()];
    sprintf(strBuf, "2.3 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    sprintf(strBuf, "3.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    sprintf(strBuf, "4.0 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode, deadtimeSettingChanA,
            dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    dutyCycleSettingChanA = Sm41.dutyCycleSetting(ChanA);
    prescale = prescaleStr[Sm41.prescaler()];
    sprintf(strBuf, "4.1 %luHz %s md:%lu dtA:%hu dcA:%hhu%%", pwmFrequency, prescale, pwmMode, deadtimeSettingChanA,
            dutyCycleSettingChanA);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    prescale = prescaleStr[Sm42.prescaler()];
    sprintf(strBuf, "4.2 %luHz %s md:%lu dtA:%hu dcA:%hhu%% dtB:%hu dcB:%hhu%%", pwmFrequency, prescale, pwmMode,
            deadtimeSettingChanA, dutyCycleSettingChanA, deadtimeSettingChanB, dutyCycleSettingChanB);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
    ...
    sprintf(strBuf, "IP: %d.%d.%d.%d", Ethernet.localIP()[0], Ethernet.localIP()[1], Ethernet.localIP()[2],
            Ethernet.localIP()[3]);
    writeLog(strBuf);
    memset(strBuf, 0, sizeof(strBuf)); // Clear buffer
}
...
void saveConfiguration(const char *filename, const MainConfig &config) {
    ...
    JsonObject Config_AsymmetricInduction = doc[F("Config")][F("AsymmetricInduction")].to<JsonObject>();
