...
void configureModule4() {
    ...
    */
    ...
    Serial.print("SM42 PWM Frequency: ");
    Serial.println(config.Pwm.Tm4.Sm42.PwmFrequency);

    const uint8_t prescalerIndex = calculateBestPrescaler(config.Pwm.Tm4.Sm42.PwmFrequency);
    const uint8_t selectedPrescaler = PrescalerValues[prescalerIndex];

    Serial.print("Best Prescaler Index: ");
    Serial.println(prescalerIndex);

    Serial.print("Selected Prescaler: ");
    Serial.println(selectedPrescaler);

    const float32_t dutyCycleA = static_cast<float32_t>(config.Pwm.Tm4.Sm42.ChannelA.DutyCycle) / 65536;
    constexpr uint16_t preShiftNanos = 500; // B turns on 500 ns before A turns off
    constexpr uint16_t postShiftNanos = 500; // B turns off 500 ns before A turns on

    // Calculate PWM period and duty cycle in ticks
    constexpr int32_t ticksPerCycle = 1000000000;
    constexpr float32_t ticksPerCycleFloat = static_cast<float32_t>(ticksPerCycle);
    const int32_t periodTicks = F_BUS_ACTUAL / config.Pwm.Tm4.Sm42.PwmFrequency;
    const uint16_t pulseTicksA = periodTicks * dutyCycleA;
    const uint16_t preShiftTicks = (F_BUS_ACTUAL / ticksPerCycleFloat) * preShiftNanos;
    const uint16_t postShiftTicks = (F_BUS_ACTUAL / ticksPerCycleFloat) * postShiftNanos;
    const uint16_t periodStart = 0;
    const uint16_t periodEnd = periodTicks - 1;
    const uint16_t startChanA = periodStart;
